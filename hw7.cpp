...
void changeLibraryType (Room* room, int newLibraryTpe) {
	if (newLibraryTpe == 0) {
		room -> lib = hayden;
	} else {
		room -> lib = noble;
...
void save(string fileName)
{
	...
	if ( fileVar.is_open()) {
		tempList = list;
		fileVar << counter << endl;

		while (tempList != NULL) {
			fileVar << tempList -> room -> getName() << endl;
			fileVar << tempList -> room -> getNo() << endl;
			libraryType lib = tempList -> room -> getLibraryType();
